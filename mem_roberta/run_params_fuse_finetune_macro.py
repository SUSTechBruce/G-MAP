...
from fuse_mem_trainer import EvalPrediction, Trainer, set_seed
from util import GlueDataset
from config_macro import glue_compute_metrics, glue_tasks_num_labels
...
def main():
    ...
    parser.add_argument("--general_weight_alpha", default=None, type=float, required=True, help="general weight parameter alpha")
    parser.add_argument("--domain_weight_alpha", default=None, type=float, required=True, help="domain roberta parameter alpha")
    ...
    # fusion then finetune
    domain_model_path = args.domain_model + '/pytorch_model.bin'
    general_model_path = args.general_model + '/pytorch_model.bin'
    
    
    models_paths = [general_model_path, domain_model_path]
    nums_models = len(models_paths)
    
    for i, model_path in enumerate(models_paths):
        print('Start to fusing the parameters of models')
        
        assert os.path.exists(model_path)
        state_dict = torch.load(model_path)
        if i == 0:
            average_fusion = {k: v * args.general_weight_alpha for k, v in state_dict.items()}
        else:
            average_fusion = {k: v * args.domain_weight_alpha + average_fusion[k] for k, v in state_dict.items()}
            
    
    model = RobertaForSequenceClassification.from_pretrained(
        pretrained_model_name_or_path=None,
        state_dict=average_fusion, # average the parameters from the 
        from_tf=False,
        config=config,
        cache_dir=args.cache_dir,
    )
    print('******************* Loading fusion model success ! ***************************')
    
    

    # Loading model for general memory model 
    # # ignore these codes for the fusion experiments
    general_model_path = args.general_model + '/pytorch_model.bin'
        ...
        for eval_dataset in eval_datasets:
            ...
            if trainer.is_world_process_zero():
                with open(output_eval_file, "w") as writer:
                    ...
                    print("***** Eval results {} *****".format(eval_dataset.args.task_name))
        ...
        for test_dataset in test_datasets:
            ...
            if trainer.is_world_process_zero():
                with open(output_test_file, "w") as writer:
                    ...
                    print("***** Test results {} *****".format(test_dataset.args.task_name))
                    for key, value in metrics.items():
                        logger.info("  %s = %s", key, value)
                        print("  %s = %s", key, value)
                        writer.write("%s = %s\n" % (key, value))
