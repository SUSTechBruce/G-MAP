...
class Trainer:
    ...
    def __init__(
        ...
        mem_model: PreTrainedModel = None,
        args = None,
        ...
        **kwargs,
        ...
        # memory model
        self.mem_model = mem_model.to(args.device) if model is not None else None
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        loss = self.compute_loss(model, mem_model, inputs)
    ...
    def compute_loss(self, model, mem_model, inputs):
        ...
        # position ids is not necessary here

        # print('Test whether the model is pretrained model', isinstance(self.model, nn.Module))
        ...
        # with torch.no_grad():
        # test = gradcheck(mem_model, input_ids)
        mem_outputs = mem_model(
                input_ids=input_ids,
        ...
        # print('Require gradient', mem_model.requires_grad)
        # output_attentions = [attention.detach() for attention in mem_outputs[3]]
    ...
    def prediction_step(
        ...
        # output_attentions = [attention.detach() for attention in mem_outputs[3]]
        mem_states = [hidden_state.detach() for hidden_state in mem_outputs[2][:-1]]
        # print('The mem_states are:', len(mem_states))
        # print('The shape of the hidden state', mem_states[0].shape)
        # print('The shape of attention ', output_attentions[1].shape)
