...
def _glue_convert_examples_to_features(
    ...
    def label_from_example(example: InputExample) -> Union[int, float, None]:
        ...
        if output_mode == "classification":
            try:
                return_result = label_map[example.label]
            except:
                print('Error')
                return_result = 0
...
class LineByLineTextDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, Ngram_dict, file_path: str, block_size: int):
        ...
        for i in range(len(examples)):
            ...
            # if i < 5:
            #     print('Test for tokenizer.tokenize', tokens_a)
...
class GlueDataset(Dataset):
    ...
    def __init__(
        ...
        print('What happen', lock_path)
        run = True
        if run:
            print('start here 1')
            stop_cache=False
            if os.path.exists(cached_features_file) and not args.overwrite_cache and stop_cache:
                print('here is ok')
                start = time.time()
