...
from modeling import PreTrainedModel, RobertaPreTrainedModel
...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None, trial = None):
        """
         inputs = {'input_ids': input_ids,
                  ...
                  'vocab_attention_mask': ngram_mask_array,
                  'vocab_token_type_ids': ngram_seg_ids,
                  'vocab_position_matrix': ngram_positions_matrix,
                  }
        feature = InputFeatures(**inputs, label=labels[i])
        """

        # Data_loader and number of training steps
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            epoch_pbar = tqdm(epoch_iterator, desc="Iteration", disable=disable_tqdm) # here is the tqdm
            for step, inputs in enumerate(epoch_iterator):
                ...
                tr_loss += self.training_step(model, inputs) # down here
    ...
    def compute_loss(self, model, inputs):
        ...
        # input mem_states, here to choose best layer to execute
        outputs = model(
            **inputs,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            mem_attention_mask=mem_attention_mask,
            mem_states=mem_states) # pay attention here
    ...
    def evaluate(self, general_model: nn.Module, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        eval_dataloader = self.get_eval_dataloader(eval_dataset)
        output = self.prediction_loop(eval_dataloader, general_model, description="Evaluation")

        return output

    def output_general_model(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        eval_dataloader = self.get_eval_dataloader(eval_dataset)
        general_model = self.general_prediction_loop(eval_dataloader, description="Evaluation")

        return general_model


    
    def general_prediction_loop(self, dataloader: DataLoader, description: str, prediction_loss_only = None):
        ...
        print('########################################### output fine-tune general model ###############################################')
        return model


    def prediction_loop(self, dataloader: DataLoader, general_model:nn.Module, description: str, prediction_loss_only = None):
        ...
        if self.args.n_gpu > 1:
            model = torch.nn.DataParallel(model)
            general_model = torch.nn.DataParallel(model)
        else:
            model = self.model
        ...
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            loss, logits, labels = self.prediction_step(model, general_model, inputs, prediction_loss_only)
    ...
    def prediction_step(
        self, model: nn.Module, general_model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], prediction_loss_only: bool
        ...
        with torch.no_grad():
            ...
            # add new mem_states in the inference in evaluation
            ...
            **inputs,
            ...
            if has_labels:
                ...
                general_loss = general_outputs[0].mean().item()
                general_logits = general_outputs[1:2]
                ...
                logits = outputs[:]
                general_logits = general_outputs[:]
            if self.args.past_index >= 0:
        ...
        general_logits = tuple(logit.detach() for logit in general_logits)
        if len(logits) == 1:
            logits = logits[0]
            general_logits = general_logits[0]
        ...
        print('general logit: ', general_logits[0, :])
        return (loss, logits + general_logits, labels)
